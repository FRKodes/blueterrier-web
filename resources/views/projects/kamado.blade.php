@extends('app')

@section('content')
	<div class="container-fluid">
		<div class="row inner-banner-detail kamado"></div>
	</div>

	<div class="container">
		<div class="row">
			
			<div class="project-info col-sm-10 col-sm-offset-1">
				<h1 class="azul text-center">KAMADO</h1>
				<p>
					Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime, veniam dolor deserunt non iusto, obcaecati maiores, rem rerum a repudiandae reprehenderit. Iste repudiandae qui sequi corrupti, aspernatur error vero nostrum.
				</p>
			</div>

			<div class="item-detail">
				<figure><img src="/images/kamado.jpg" alt="Proyecto kamado Grill"></figure>
			</div>

			<div class="item-detail">
				<figure><img src="/images/kamado.jpg" alt="Proyecto kamado Grill"></figure>
			</div>

			<div class="item-detail">
				<figure><img src="/images/kamado.jpg" alt="Proyecto kamado Grill"></figure>
			</div>

			<div class="item-detail">
				<figure><img src="/images/agua-maldita-botella.jpg" class="bottle" alt="agua maldita botella"></figure>
			</div>
		</div>
	</div>

@stop